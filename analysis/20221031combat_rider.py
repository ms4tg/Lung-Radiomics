...
p_data = pd.read_csv("rider_data.csv", header = 0)
...
for index, feature in columns.items():
    ...
    axis.scatter(p_volume, p_data.iloc[index], s=2, c='b', label = "Rider")

    x1 = np.linspace(min(volume), max(volume), 1000)
    #find best fit line
    coefs=poly.polyfit(volume,data.iloc[index], 1)
    ffit = poly.Polynomial(coefs)
    axis.plot(x1, ffit(x1), c = "r", linewidth=1)
    ...
    R_sq1 = corr**2
    axis.text(0.1,0.9, r'$R^2=${:.4f}'.format(R_sq1), c = "r", fontsize=15, transform=axis.transAxes)

    x2 = np.linspace(min(p_volume), max(p_volume), 1000)
    coefs=poly.polyfit(p_volume,p_data.iloc[index], 1)
    ffit = poly.Polynomial(coefs)    # instead of np.poly1d
    axis.plot(x2, ffit(x2), c = "b", linewidth=1)
    ...
    corr = corr_matrix[0,1]
    R_sq2 = corr**2
    axis.text(0.1,0.8, r'$R^2=${:.4f}'.format(R_sq2), c = 'b', fontsize=15, transform=axis.transAxes)

    r_value.append((R_sq1,R_sq2))
    #at = AnchoredText(
    ...
    plt.savefig("/media/sf_research/krishni/output/20221031combat_rider/"+str(index)+str(feature)+".png")
    print("graph plotted:",index)

df = pd.DataFrame(r_value, columns =['Combat',"Rider"],index = columns)
df.to_csv('r_value_rider.csv', index=True, header=True, sep=',')
