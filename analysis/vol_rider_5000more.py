...
FilePath="/nv/vol141/phys_nrf/Menglin/"
...
rider_patients = list(range(1,25))

blacklist=["DA","MJ","BB","HR","HI","HJ","PC"]
dates=pd.read_csv(FilePath+"dataset_new/"+"Dates.csv")
patients=list(dates.columns)
for i in blacklist:
    if i in patients:
        patients.remove(i)
...
rider_ITV={p:pd.read_csv(FilePath+"rider_data/"+str(p)+"/GTVp_test_auto.csv") for p in rider_patients}
ITV={p:pd.read_csv(FilePath+"dataset_new/"+p+"/ITV_new.csv") for p in patients}
...
for k in keys:
    X,y=[],[]
    for p in rider_patients:
        #X+=d[p]
        volume = list(rider_ITV[p].iloc[35])[1:]
        volume = [float(i) for i in volume]
        X+=volume
        #Normalize to Pre, might want to change this
        add=list(rider_ITV[p].iloc[k])[1:]
...
for k in keys:
    ...
    for p in patients:
        ...
        ITV[p]=ITV[p][(ITV[p]['original_shape_MeshVolume'])>5000]
        #print(ITV[p].head())
        #ITV[p]=ITV[p].T
...
for i in keys:
    ...
    axis.scatter(rider_ITVdata[i][0],rider_ITVdata[i][1], s=2, c='r')
    fit=np.polyfit(rider_ITVdata[i][0], rider_ITVdata[i][1], 1)
    p=np.poly1d(fit)
    axis.plot(rider_ITVdata[i][0], p(rider_ITVdata[i][0]), "r", linewidth=1)
    ...
    axis.legend(["ITV", "rider_ITV"],loc='upper right')
    plt.savefig("/nv/vol141/phys_nrf/Menglin/output/220908vol_rider_5000more.py/"+str(i)+".png")
